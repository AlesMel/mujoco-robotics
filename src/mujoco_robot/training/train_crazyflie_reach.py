"""Train PPO on the Crazyflie reach-random-goals task.

Usage::

    python -m mujoco_robot.training.train_crazyflie_reach \
        --cfg-name crazyflie_reach_dense_stable
"""
...
def train_crazyflie_reach_ppo(
    ...
    """Quick-start PPO training on the Crazyflie reach task."""
    ...
    print(f"\n{'='*50}")
    print("  Crazyflie REACH training config")
    ...
    def make_env(rank: int):
        def _init():
            ...
            return Monitor(make_crazyflie_reach_gymnasium(cfg))
        return _init
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train PPO on Crazyflie reach-random-goals task."
    )
