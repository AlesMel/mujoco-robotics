"""Train PPO on the Crazyflie hover task.

Usage::

    python -m mujoco_robot.training.train_crazyflie \
        --cfg-name crazyflie_hover_dense_stable
"""
...
def train_crazyflie_ppo(
    total_timesteps: int = 4_000_000,
    n_envs: int = 16,
    log_dir: str = "runs",
    log_name: str = "crazyflie_hover_ppo",
    ...
    """Quick-start PPO training on the Crazyflie hover task."""
    ...
    print(f"\n{'='*50}")
    print("  Crazyflie hover training config")
    ...
    def make_env(rank: int):
        def _init():
            ...
            return Monitor(make_crazyflie_hover_gymnasium(cfg))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train PPO on Crazyflie hover task.")
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Render training in real time (forces n_envs=1 and uses DummyVecEnv).",
    )
