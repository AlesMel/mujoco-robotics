...
# ---------------------------------------------------------------------------
_DEFAULT_MODEL = str(
    Path(__file__).resolve().parent.parent.parent / "robots" / "ur5e.xml"
)
...
class SlotSorterGymnasium(gymnasium.Env):
    ...
    time_limit : int
        Maximum episode length for the wrapped raw env.
    model_path : str
        Robot MJCF path.
    actuator_profile : str
        Named actuator profile from ``mujoco_robot.robots.actuators``.
    **env_kwargs
        Forwarded to :class:`URSlotSorterEnv`.
    """
...
class URSlotSorterEnv:
    ...
    def __init__(
        ...
        # Passive damping / friction + actuator bounds/gain
        configure_position_actuators(
            self.model,
            actuator_handles,
            min_damping=8.0,
    ...
    def _observe(self) -> np.ndarray:
        ...
        for qpos_adr in self._robot_qpos_ids:
            obs_parts.append(np.array([qpos[qpos_adr]]))
        for dof_adr in self.robot_dofs:
            obs_parts.append(np.array([qvel[dof_adr]]))
